...
import {
  ...
  genderSchema, colorSchema, sizeSchema, brandSchema,
  categorySchema, collectionSchema, productSchema, productVariantSchema, productImageSchema,
...
async function seed() {
  try {
    ...
    const genderRows = [
      genderSchema.insert.parse({ label: 'Men', slug: 'men' }),
      genderSchema.insert.parse({ label: 'Women', slug: 'women' }),
      genderSchema.insert.parse({ label: 'Unisex', slug: 'unisex' }),
    ];
    ...
    ].map((c) => colorSchema.insert.parse(c));
    ...
    ].map((s) => sizeSchema.insert.parse(s));
    ...
    const brand = brandSchema.insert.parse({ name: 'Nike', slug: 'nike', logoUrl: undefined });
    {
    ...
    ].map((c) => categorySchema.insert.parse(c));
    ...
    const collectionRows = [
      collectionSchema.insert.parse({ name: "Summer '25", slug: 'summer-25' }),
      collectionSchema.insert.parse({ name: 'New Arrivals', slug: 'new-arrivals' }),
    ];
    ...
    for (let i = 0; i < productNames.length; i++) {
      ...
      const product = productSchema.insert.parse({
        name,
        description: desc,
      ...
      for (const color of colorChoices) {
        for (const size of sizeChoices) {
          ...
          const variant = productVariantSchema.insert.parse({
          ...
          const retV = await db.insert(productVariants).values(variant as InsertProductVariant).returning();
      ...
      try {
        cpSync(src, dest);
        const img: InsertProductImage = productImageSchema.insert.parse({
          productId: insertedProduct.id,
