...
export function parseQuery(search: string): QueryObject {
  // `search` expected without leading "?" (useSearchParams().toString())
...
export function toggleArrayParam(pathname: string, currentSearch: string, key: string, value: string): string {
  const current = parseQuery(currentSearch);
  const curVal = current[key];
  const arr = new Set<string>(Array.isArray(curVal) ? (curVal as string[]) : curVal ? [String(curVal)] : []);
...
export function setParam(pathname: string, currentSearch: string, key: string, value: string | number | null | undefined): string {
...
/* Product-specific helpers and normalised filters */
...
export function parseFilterParams(sp: Record<string, string | string[] | undefined>): NormalizedProductFilters {
  const getArr = (k: string) => {
    ...
    const arr2 = Array.isArray(v2) ? v2.map(String) : v2 === undefined ? [] : [String(v2)];
    return [...arr1.filter(Boolean), ...arr2.filter(Boolean)];
  };
...
export function buildProductQueryObject(filters: NormalizedProductFilters) {
  // simple identity: used by client components if needed
  return filters;
}